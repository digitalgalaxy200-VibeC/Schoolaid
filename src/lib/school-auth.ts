...
export async function verifySchoolAdmin(): Promise<{ authorized: boolean; school_id: string | null; userId: string | null }> {
  ...
  try {
    ...
    if (payload.role === "school_admin" && payload.school_id) {
      return { authorized: true, school_id: payload.school_id as string, userId: payload.sub as string };
    }
  } catch {}
...
export async function verifyTeacher(): Promise<{ authorized: boolean; school_id: string | null; userId: string | null }> {
  const cookieStore = await cookies();
  const session = cookieStore.get("schoolaid-session")?.value;
  if (!session) return { authorized: false, school_id: null, userId: null };
