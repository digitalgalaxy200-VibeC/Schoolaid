...
type TermResult = {
  ...
  term_name: string;
  session_name: string;
  subjects: { subject_name: string; total_score: number; grade: string; remark: string }[];
};

export default function StudentDashboard() {
  const router = useRouter();
  const [sessions,        setSessions]        = useState<{ id: string; session_name: string }[]>([]);
  ...
  return (
    <div className="space-y-6">
      ...
      <Card variant="bordered" className="shadow-sm space-y-4">
        <div>
          <label className="block text-small font-semibold text-text-secondary mb-1">Academic Session</label>
          <select
        ...
        {selectedSession && (
          <div>
            <label className="block text-small font-semibold text-text-secondary mb-1">Term</label>
            <select
              value={selectedTerm}
      ...
      {!loadingResults && selectedTerm && !results && (
        <Card variant="bordered" className="text-center py-10">
          <p className="text-text-muted">Results for this term have not been published yet.</p>
          <p className="text-caption text-text-muted mt-1">Check back after your teacher publishes them.</p>
        </Card>
      )}
      ...
      {results && results.subjects.length > 0 && (
        <Card variant="bordered" className="shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h2 className="text-h3 font-bold">{results.term_name} — {results.session_name}</h2>
              <p className="text-caption text-text-muted mt-0.5">Published result — official snapshot</p>
            </div>
            <button
              onClick={() => router.push(`/student/results?term=${results.term_id}`)}
            ...
            <table className="w-full text-small">
              <thead>
                <tr className="border-b border-border">
                  <th className="text-left py-2 pr-4 font-semibold text-text-secondary">Subject</th>
                  <th className="text-center py-2 px-4 font-semibold text-text-secondary">Total</th>
                  <th className="text-center py-2 px-4 font-semibold text-text-secondary">Grade</th>
                  <th className="text-left py-2 pl-4 font-semibold text-text-secondary">Remark</th>
                </tr>
              </thead>
              <tbody>
                {results.subjects.map((s, i) => (
                  <tr key={i} className="border-b border-border last:border-0">
                    ...
                    <td className="py-3 px-4 text-center font-bold text-primary">{s.total_score}</td>
                    <td className="py-3 px-4 text-center">
                      <Badge variant={s.grade === "A" ? "success" : s.grade === "F" ? "error" : "default"}>
                        {s.grade}
                      </Badge>
                    </td>
                    <td className="py-3 pl-4 text-text-secondary">{s.remark}</td>
                  </tr>
                ))}
              </tbody>
            </table>
