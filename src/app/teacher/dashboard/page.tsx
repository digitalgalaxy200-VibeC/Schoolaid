  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {data.classes.length === 0 ? (
        <Card variant="bordered" className="shadow-sm"><p className="text-small text-text-muted py-8 text-center">No classes assigned yet.</p></Card>
      ) : (
        <div className="grid grid-cols-1 tablet:grid-cols-2 gap-4">
          {data.classes.map((c) => (
            <Card key={c.id} variant="bordered" className="shadow-sm hover:shadow-md transition-shadow cursor-pointer"
              onClick={() => router.push(`/teacher/scores?class=${c.id}`)}>
              <div className="p-5">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-h3 font-bold">{c.name}</h3>
                  {c.role && <Badge variant={c.role === "primary" ? "success" : "default"}>{c.role === "primary" ? "Primary" : "Assistant"}</Badge>}
                </div>
                {c.grade && <p className="text-caption text-text-muted mb-3">{c.grade}</p>}
                ...
                {c.subjects.length > 0 && (
                  <div className="mt-3 pt-3 border-t border-border">
                    <p className="text-caption text-text-muted mb-1">Subjects:</p>
