...
export function ReviewDetail({ detail, onDone }: { detail: Detail; onDone: () => void }) {
  ...
  const buildReportData = (s: Student): ReportCardData => {
    ...
    const adminRemark = localSaved !== undefined
      ? (localSaved || "")
  ...
  if (viewingStudent) {
    ...
    const autoGeneratedComment = (() => {
      ...
      return `${firstName} had a satisfactory result.`;
    })();

    const displayAdminComment = existingAdminComment || autoGeneratedComment;

    const persistAdminComment = async (comment: string) => {
    ...
    const saveAdminComment = async () => {
      const comment = (adminCommentText || displayAdminComment).trim();
      if (!comment) {
        setError("Please enter a remark before saving.");
        return;
      }
      await persistAdminComment(comment);
    };

    const generateAdminComment = async () => {
      const generated = autoGeneratedComment.trim();
      if (!generated) {
        setError("No remark could be generated for this student yet.");
        return;
      }
      // Explicit user request: populate the field AND persist so the report
      // card reads the exact same value (single source of truth).
      setAdminCommentText(generated);
      await persistAdminComment(generated);
    };
      ...
      <div className="space-y-4">
        ...
        <Card variant="default" className="p-4 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <Button variant="ghost" size="sm" onClick={generateAdminComment} loading={savingComment}>✨ Use AI for Generation</Button>
        ...
        <div className="overflow-x-auto border border-border rounded-sm bg-surface">
          <table className="w-full text-small">
            ...
            <tbody>
              {students.map((s, i) => {
                const sum = summaries.get(s.id);
                const offered = sum?.totals.filter((t) => t.total !== null && t.total >= 1 && t.total <= 100) || [];
