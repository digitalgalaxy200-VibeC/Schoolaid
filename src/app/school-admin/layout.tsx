...
const navItems = [
  ...
  { label: "Grading Scale",href: "/school-admin/grading" },
  { label: "Reports",      href: "/school-admin/reports" },
    ...
    <div className="min-h-screen bg-bg flex">
      ...
      <aside className="hidden tablet:flex w-60 bg-surface border-r border-border flex-col shrink-0">
        <div className="p-5 border-b border-border">
          ...
          <p className="text-caption text-text-muted mt-0.5 truncate">{schoolName}</p>
        </div>
        ...
        <div className="p-4 border-t border-border">
          ...
          <button
            ...
            className="w-full text-left text-small text-error hover:underline"
          >
            Sign Out
          </button>
        </div>
        ...
        <div className="tablet:hidden fixed top-12 left-0 right-0 z-30 bg-surface border-b border-border shadow-md p-3 space-y-0.5">
          {navItems.map((item) => (
            <button
              ...
              className="w-full text-left px-4 py-2.5 rounded-sm text-small font-medium text-text-secondary hover:bg-bg hover:text-text-primary"
            >
              {item.label}
            </button>
          ))}
          <hr className="border-border my-2" />
          <button onClick={handleSignOut} className="w-full text-left px-4 py-2 text-small text-error">
            Sign Out
          </button>
        </div>
      )}
      ...
      <main className="flex-1 overflow-auto tablet:mt-0 mt-12">
        <div className="max-w-6xl mx-auto px-4 tablet:px-6 py-6">{children}</div>
      </main>
    </div>
  );
}
