...
export async function POST(request: Request) {
  ...
  const authRes = await fetch(`${process.env.NEXT_PUBLIC_SUPABASE_URL}/auth/v1/admin/users`, {
    ...
    body: JSON.stringify({ email, password, email_confirm: true, user_metadata: { full_name: fullName, role: "teacher", school_id } }),
  });
  ...
  await supabase.from("profiles").upsert({ id: authData.user.id, school_id, full_name: fullName, email, role: "teacher" });
  const { data: teacher, error } = await supabase.from("teachers").insert({
    school_id, profile_id: authData.user.id, employee_id: `T-${Date.now().toString(36)}`, qualification,
