...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (template_id) {
      ...
      await supabase.from("psychomotor_templates").update({ name }).eq("id", template_id).eq("school_id", school_id);
    } else {
      const { data, error } = await supabase.from("psychomotor_templates").insert({ school_id, name }).select().single();
      if (error) throw error;
      template_id = data.id;
    }

    // Replace relations (template_id verified/owned by this school above)
    await supabase.from("class_psychomotor_templates").delete().eq("school_id", school_id).eq("template_id", template_id);
    ...
    if (class_ids.length > 0) {
      ...
      const { error: clsErr } = await supabase.from("class_psychomotor_templates").insert(class_ids.map((c: string) => ({ school_id, class_id: c, template_id })));
    ...
    if (rows.length > 0) {
      ...
      if (validRows.length > 0) {
        const { error: rowErr } = await supabase.from("psychomotor_rows").insert(validRows.map((r: any) => ({
